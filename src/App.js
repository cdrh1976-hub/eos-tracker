...
const SESSION_TYPES = ["Focus Day", "Annual", "Quarterly", "Other"];
...
const initialClients = [
  {
    ...
    sessions: [
      { id: 1, type: "Focus Day", date: "2025-01-15", grossFee: 3500, travelAirfare: 420, travelHotel: 180, travelPerDiem: 200, paid: true },
      { id: 2, type: "Quarterly", date: "2025-03-10", grossFee: 3500, travelAirfare: 0, travelHotel: 0, travelPerDiem: 0, paid: true },
      { id: 3, type: "Quarterly", date: "2025-06-12", grossFee: 3500, travelAirfare: 420, travelHotel: 180, travelPerDiem: 200, paid: false },
    ]
  },
  {
    id: 2,
    ...
    notes: "",
    sessions: [
      { id: 1, type: "Focus Day", date: "2025-02-20", grossFee: 4000, travelAirfare: 300, travelHotel: 0, travelPerDiem: 0, paid: true },
      { id: 2, type: "Quarterly", date: "2025-05-15", grossFee: 4000, travelAirfare: 0, travelHotel: 0, travelPerDiem: 0, paid: true },
    ]
  }
];
...
function Badge({ color, children }) {
  ...
  return (
    <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 20, letterSpacing: "0.04em", ...Object.fromEntries(colors[color].split(";").map(p => p.split(":"))) }}>
      {children}
    </span>
  );
}

const emptySession = { type: "Focus Day", date: "", grossFee: "", travelAirfare: "", travelHotel: "", travelPerDiem: "", paid: false };
const emptyClient = { name: "", leadSource: "self", paymentMethod: "Per Session", annualSessions: 4, sessionRate: "", notes: "" };

export default function App() {
  const [clients, setClients] = useState(initialClients);
  const [view, setView] = useState("dashboard"); // dashboard | clients | client-detail
  ...
  const stats = useMemo(() => {
    ...
    clients.forEach(client => {
      client.sessions.forEach(s => {
        const { netPayout, travelTotal, totalDeductions } = calcPayout(s, client);
        totalGross += s.grossFee;
        totalNet += netPayout;
        totalTravel += travelTotal;
        if (s.paid) totalPaid += netPayout; else totalUnpaid += netPayout;
      });
    });
    // Cash forecast: unpaid sessions + future monthly payments
  ...
  function togglePaid(clientId, sessionId) {
    setClients(prev => prev.map(c => c.id === clientId
      ? { ...c, sessions: c.sessions.map(s => s.id === sessionId ? { ...s, paid: !s.paid } : s) }
      : c
    ));
  }
    ...
    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", background: "#0f0f13", minHeight: "100vh", color: "#e8e6e1" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag { display: inline-block; font-size: 10px; font-weight: 700; padding: 2px 7px; border-radius: 20px; letter-spacing: 0.05em; }
      ...
      <div style={{ padding: "28px 32px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        {view === "dashboard" && (
          <div>
            ...
            {/* All sessions table */}
            <div className="card">
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 16, color: "#9e9cb5" }}>ALL SESSIONS</div>
                ...
                <tbody>
                  {clients.flatMap(client =>
                    client.sessions.map(s => {
                      ...
                      return (
                        <tr key={`${client.id}-${s.id}`} className="row-hover" style={{ borderBottom: "1px solid #1a1a22", transition: "background 0.1s" }}>
                          ...
                          <td style={{ padding: "10px 10px" }}>
                            <button className="btn btn-sm" onClick={() => togglePaid(client.id, s.id)}
                              style={{ background: s.paid ? "#1a2e1a" : "#2e1a1a", color: s.paid ? "#60efb8" : "#ef9f60", border: `1px solid ${s.paid ? "#2a4a2a" : "#4a2a2a"}`, borderRadius: 20, fontSize: 11, fontWeight: 700, padding: "3px 10px", cursor: "pointer" }}>
                              {s.paid ? "Paid" : "Unpaid"}
                            </button>
                          </td>
                        </tr>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {filteredClients.map(client => {
                ...
                const unpaidNet = client.sessions.filter(s => !s.paid).reduce((sum, s) => sum + calcPayout(s, client).netPayout, 0);
                return (
        ...
        {view === "client-detail" && selectedClient && (() => {
          ...
          const paidNet = selectedClient.sessions.filter(s => s.paid).reduce((s, sess) => s + calcPayout(sess, selectedClient).netPayout, 0);
          return (
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#9e9cb5", textTransform: "uppercase", letterSpacing: "0.06em" }}>Sessions</span>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {selectedClient.sessions.map(s => {
                      ...
                      const deduction = referralDeduction + commissionDeduction;
                      return (
                        <tr key={s.id} className="row-hover" style={{ borderBottom: "1px solid #1a1a22" }}>
                          <td style={{ padding: "10px 10px", color: "#6b6986", fontFamily: "'DM Mono', monospace", fontSize: 12 }}>{s.date}</td>
                          <td style={{ padding: "10px 10px" }}><span style={{ fontSize: 12, color: s.type === "Focus Day" ? "#c8f04a" : "#9e9cb5" }}>{s.type}</span></td>
                          <td style={{ padding: "10px 10px", fontFamily: "'DM Mono', monospace" }}>{fmt(s.grossFee)}</td>
                          <td style={{ padding: "10px 10px" }}>
                          ...
                          <td style={{ padding: "10px 10px", fontFamily: "'DM Mono', monospace", fontSize: 12, color: "#6b6986" }}>{travelTotal > 0 ? fmt(travelTotal) : "—"}</td>
                          <td style={{ padding: "10px 10px" }}>
                            <button onClick={() => togglePaid(selectedClient.id, s.id)}
                              style={{ background: s.paid ? "#1a2e1a" : "#2e1a1a", color: s.paid ? "#60efb8" : "#ef9f60", border: `1px solid ${s.paid ? "#2a4a2a" : "#4a2a2a"}`, borderRadius: 20, fontSize: 11, fontWeight: 700, padding: "3px 10px", cursor: "pointer" }}>
                              {s.paid ? "Paid" : "Unpaid"}
                            </button>
                          </td>
